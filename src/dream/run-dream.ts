...
import {
  HonchoRawSignalIngestionImplementation,
  HonchoSignalIngestionImplementation,
} from "../stages/honcho-signal-ingestion-stage.js";
...
type SignalStageFactory = new (
  ...
  hooks?: {
    onInsight?: (
      context: import("../core/types.js").DreamContext,
      insight: import("../core/types.js").InsightRecord,
    ) => void;
    onSessionComplete?: (
      context: import("../core/types.js").DreamContext,
    ) => Promise<void> | void;
...
function createRunId(
  ...
  const remoteUrl = gitRead(workspaceDir, [
    "config",
    "--get",
    "remote.origin.url",
  ]);
  ...
  const branch = slug(
    gitRead(workspaceDir, ["rev-parse", "--abbrev-ref", "HEAD"]),
    "detached",
    20,
  );
...
export async function runDream(
  ...
  const copilotAdapterOptions = {
    ...
    sessionScopeMode:
      options.sessionScopeMode ?? config.copilotDebugSessionScopeMode,
  ...
  registry.registerAdapter(
    new TerminalRecordingAdapter(config.terminalCastPath),
  );
  ...
  registry.registerBackend(
    new CopilotMemoryBackend(workspaceDir, config.copilotMemoryPath),
  );
  registry.registerBackend(
  ...
  registry.registerProvider(
    new CopilotSdkProvider(config.copilotSdkProviderOptions),
  );
  ...
  const requestedSessionWorkspaceMode =
    options.sessionWorkspaceMode ?? config.copilotDebugSessionWorkspaceMode;
  const enforceWorkspaceDefault = isTruthyEnv(
    process.env.DREAM_SAFE_LOCK_WORKING_DIRECTORY,
  );
  const effectiveSessionWorkspaceMode = enforceWorkspaceDefault
    ? "workspace-default"
    : requestedSessionWorkspaceMode;
  ...
  let honchoSignalImplementation:
    | HonchoSignalIngestionImplementation
    | undefined;
  const honchoSignalStage = new SignalStageCtor(
    ...
    config.stageAgentPacks?.["stage.signal"],
    {
      onInsight: (context, insight) =>
        honchoSignalImplementation?.recordInsight(context, insight),
      onSessionComplete: (context) =>
        honchoSignalImplementation?.flushPending(context, "session"),
      onComplete: (context) => honchoSignalImplementation?.complete(context),
    },
    effectiveSessionWorkspaceMode,
  );
  honchoSignalImplementation = new HonchoSignalIngestionImplementation(
    honchoSignalStage,
    workspaceDir,
    {
      workspaceId: config.honchoWorkspaceId,
  ...
  registry.registerStageImplementation(honchoSignalImplementation);
  registry.registerStage(
    new ConsolidationStage(
      provider,
      config.stageAgentPacks?.["stage.consolidation"],
    ),
  );

  try {
    const context = buildContext(workspaceDir, runId);
    const effectiveSessionScopeMode =
      options.sessionScopeMode ?? config.copilotDebugSessionScopeMode;
    const effectiveSinceDays =
      options.sinceDays ?? config.copilotDebugLookbackDays;
    const effectivePersistState = options.persistState !== false;
    if (
      enforceWorkspaceDefault &&
      requestedSessionWorkspaceMode !== "workspace-default"
    ) {
      context.diary.push(
    ...
    context.diary.push(
      `source:copilotDebugSessionDir=${config.copilotDebugSessionDir}`,
    );
    context.diary.push(
      `config:maxSessions=${formatLimit(configuredMaxSessions)}`,
    );
    context.diary.push(
      `config:batchSessions=${formatLimit(configuredBatchSessions)}`,
    );
    context.diary.push(`config:sinceDays=${effectiveSinceDays}`);
    context.diary.push(`config:sessionScope=${effectiveSessionScopeMode}`);
    context.diary.push(
      `config:sessionWorkspace=${effectiveSessionWorkspaceMode}`,
    );
    context.diary.push(`config:minSessions=${config.minSessions}`);
    context.diary.push(
      `config:replayFromStart=${options.replayFromStart === true}`,
    );
    context.diary.push(`config:persistState=${effectivePersistState}`);
    const backupResult = await backupMemoryBeforeRun(
      workspaceDir,
      runId,
      config,
    );
    ...
    const orderedStages = config.stageOrder.map((id) =>
      registry.requireStageForSlot(id, config.stageImplementations[id]),
    );
    ...
    while (true) {
      ...
      if (config.adapterId === "adapter.copilot.debug") {
        const caps = [configuredBatchSessions, remainingSessionBudget].filter(
          (value): value is number =>
            typeof value === "number" && Number.isFinite(value) && value > 0,
        );
        copilotAdapterOptions.maxSessionsPerRun =
          caps.length > 0 ? Math.min(...caps) : undefined;
      }
      ...
      const sessions = ingest.events.filter(
      ...
      const ingestComplete = Boolean(
        ingest.progress &&
        (ingest.progress.remainingUnits === 0 ||
          ingest.progress.completionPercent >= 100),
      );
      status.update(
      ...
      if (ingest.progress) {
        ...
        if (ingest.progress.etaMinutes !== undefined) {
          context.diary.push(
            `ingest:eta_minutes=${ingest.progress.etaMinutes}`,
          );
        }
      }

      lastCursor = ingest.cursor ?? lastCursor ?? null;
      lastAdapterCheckpoint =
        ingest.checkpoint ?? lastAdapterCheckpoint ?? ingest.cursor ?? null;
      ...
      if (sessions < config.minSessions) {
        status.update(
          `skipping pipeline cycle=${cycle} sessions=${sessions} minSessions=${config.minSessions}`,
        );
      ...
      await status.track(
        `saving memories cycle=${cycle}`,
        backend.save(context.memories),
      );
      if (ingestComplete) break;
    }

    await status.track(
      "writing provider docs artifacts (workspace storage)",
      writeProviderDocs(context, provider, config),
    );
