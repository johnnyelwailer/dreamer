...
export async function runDream(workspaceDir: string, options: RunDreamOptions = {}): Promise<void> {
  ...
  try {
    ...
    const ingest = await status.track(`ingesting ${config.adapterId}${options.replayFromStart ? " (replay from start)" : ""}`, adapter.ingest(adapterCheckpoint));
    ...
    status.update(`ingest complete events=${context.events.length} sessions=${context.metrics.sessionsProcessed}`);
    ...
    if (sessions < config.minSessions) {
      status.done(`skipping pipeline sessions=${sessions} minSessions=${config.minSessions}`);
    ...
    const finalContext = await runPipeline(context, orderedStages, status);
    await status.track("writing provider docs", writeProviderDocs(finalContext, provider, config));
    await status.track("saving memories", backend.save(finalContext.memories));
    if (options.persistState !== false) {
      await status.track("persisting state", state.write({ cursor: ingest.cursor ?? null, adapterCheckpoint: ingest.checkpoint ?? ingest.cursor ?? null, adapterProgress: ingest.progress, lastRunAt: finalContext.nowIso }));
    }
    status.done(`finished run=${runId}`);
  } finally {
